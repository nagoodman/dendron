(ns hbasey.testdata
  ...
  (:use [hbasey.dcubeutils]))

(def sparse-data ^{:dimensions [:airline [:year :qtr] :origin] :measure :qty}
  {[:alaska [2010 :q1] :ca] 2
   [:alaska [2010 :q1] :wa] 2
   [:alaska [2010 :q2] :ca] 5
   [:alaska [2010 :q2] :or] 4
...
; alaska only, states: ca, or, wa. [year-qtrs]: 2010-q1, q2, q3, q4. 2011-q1, q2
; thus N=10
(def sparse-data2d ^{:dimensions [[:year :qtr] :origin] :measure :qty}
  {[[2010 :q1] :ca] 2
   [[2010 :q1] :wa] 2
   [[2010 :q2] :ca] 5
   [[2010 :q2] :or] 4
...
(def sparse-cube2d ^{:N 10}
  {[[2010 :q1] :ca] 2
   ...
   ; (they form a chain so updates work)
   [[2010 :q4] :ca] {:goto [[2010 :q3] :ca]}
   [[2011 :q1] :ca] {:goto [[2010 :q4] :ca]}
   })

; stored in separate column family
(def sparse-cube2d-md
  {[[2010 :q1] :ca] {} ; empty, assuming root origin
   [[2010 :q1] :wa] {:border-of [[2010 :q1] :ca]} ; with respect to anchor,not origin
   [[2010 :q2] :ca] {:border-of [[2010 :q1] :ca]}
   [[2010 :q2] :or] {:parent-anchor [[2010 :q1] :ca]} ; has a parent, thus is origin
   [[2010 :q2] :wa] {:border-of [[2010 :q2] :or]}
   [[2010 :q3] :ca] {:border-of [[2010 :q1] :ca]}
   [[2010 :q4] :or] {:anchor-of [[2010 :q2] :or]} ; with respect to origin
   [[2010 :q4] :wa] {:border-of [[2010 :q4] :or]}
   [[2011 :q1] :wa] {:parent-anchor [[2010 :q4] :or]}
   [[2011 :q2] :ca] {:anchor-of [[2010 :q1] :ca]}
   })

; now queries go in reverse until we hit a cell with no parent:
; getting from top-origin to [11-q1, wa] thus goes:
; got [11-q1, wa]=4. it has a parent, got [[2010 :q4] :or]=6. substitute
; dimension-by-dimension to get borders, get [[2011 :q1] :or]=0 and
; [[2010 :q4] :wa]=3. since [[2010 :q4] :or]=6 is an anchor of
; [[2010 :q2] :or], go there and get its parent if it has one (if no, we're done)
; its parent is [[2010 :q1] :ca]=2. again do dimension-by-dimension substitution
; and we get [[2010 :q1] :wa]=2, [[2011 :q1] :wa]=7.

(defn query-data ; query sparse-data
  [cube query]
  (let [[k v] (first (filter #(= query (first %1)) (partition 2 cube)))]
    (or v 0)))

; original data set, 2d
...
(def dumb
  (with-meta (apply hash-map (interleave (for [x (range 10) y (range 10) z (range 10)] [x y z]) (repeat 1))) {:N 10}))

(query-ddc-range dumb [0 0 0] [1 1 1])

(query-ddc-range dumb [0 0 0] [1 0 1])

(where-is-cell [1 0 1] [0 0 0] 10)

(shared-anchor (anchor-slots [0 0 0] 10) [1 0 1])
